...
/// Canonical column names for [`STATS_COLUMNS_DTYPE`].
...
/// The canonical dtype of a [`StatsColumns`] struct: 11 nullable typed columns mirroring
...
pub static STATS_COLUMNS_DTYPE: LazyLock<DType> = LazyLock::new(|| {
    ...
    let bool_ = || DType::Bool(Nullability::Nullable);
    let u64_ = || DType::Primitive(PType::U64, Nullability::Nullable);
    DType::Struct(
        StructFields::new(
            STATS_FIELDS
                .iter()
...
/// Per-node statistics stored as a [`StructArray`] with the canonical
/// [`STATS_COLUMNS_DTYPE`] schema — 11 sibling typed nullable columns, one per stat slot.
///
/// Hydration into a typed [`StatsSet`] happens at decode time via [`Self::read`], when
/// the array's dtype is known.
#[derive(Debug, Clone)]
pub struct StatsColumns(StructArray);

impl StatsColumns {
    /// Wrap a [`StructArray`], validating its dtype matches [`STATS_COLUMNS_DTYPE`].
    pub fn new(inner: StructArray) -> VortexResult<Self> {
        if inner.as_ref().dtype() != &*STATS_COLUMNS_DTYPE {
            vortex_bail!(
                "StatsColumns dtype mismatch: got {}, expected {}",
                inner.as_ref().dtype(),
                *STATS_COLUMNS_DTYPE,
            );
        }
    ...
    ///
    /// Returns `None` if every column is null at that row (no stats were persisted for
    /// this node). Proto-encoded `min` / `max` / `sum` are deserialized using
    /// `array_dtype` to recover the typed `ScalarValue`.
    ///
    ...
    pub fn read(
        ...
        // Binary stats: bytes + (for min/max) an exact tag.
        if let Some(bytes) = field("min")?.as_binary().value().cloned()
            && let Some(stat_dtype) = Stat::Min.dtype(array_dtype)
            && let Some(value) =
                ScalarValue::from_proto_bytes(bytes.as_slice(), &stat_dtype, session)?
        {
            let exact = field("min_exact")?.as_bool().value().unwrap_or(true);
            let kind = if exact {
                Precision::Exact
            } else {
                Precision::Inexact
            };
            set.set(Stat::Min, kind(value));
        }
        if let Some(bytes) = field("max")?.as_binary().value().cloned()
            && let Some(stat_dtype) = Stat::Max.dtype(array_dtype)
            && let Some(value) =
                ScalarValue::from_proto_bytes(bytes.as_slice(), &stat_dtype, session)?
        {
            let exact = field("max_exact")?.as_bool().value().unwrap_or(true);
            let kind = if exact {
                Precision::Exact
            } else {
                Precision::Inexact
            };
            set.set(Stat::Max, kind(value));
        }
        if let Some(bytes) = field("sum")?.as_binary().value().cloned()
            && let Some(stat_dtype) = Stat::Sum.dtype(array_dtype)
            && let Some(value) =
                ScalarValue::from_proto_bytes(bytes.as_slice(), &stat_dtype, session)?
        {
        ...
        // Typed primitive counts.
        if let Some(v) = field("null_count")?.as_primitive().as_::<u64>() {
            set.set(Stat::NullCount, Precision::Exact(ScalarValue::from(v)));
        }
        if let Some(v) = field("nan_count")?.as_primitive().as_::<u64>() {
            set.set(Stat::NaNCount, Precision::Exact(ScalarValue::from(v)));
        }
        if let Some(v) = field("uncompressed_size_in_bytes")?
            .as_primitive()
            .as_::<u64>()
        {
            set.set(
        ...
        // Typed bool predicates.
        if let Some(v) = field("is_constant")?.as_bool().value() {
            set.set(Stat::IsConstant, Precision::Exact(ScalarValue::from(v)));
        }
        if let Some(v) = field("is_sorted")?.as_bool().value() {
            set.set(Stat::IsSorted, Precision::Exact(ScalarValue::from(v)));
        }
        if let Some(v) = field("is_strict_sorted")?.as_bool().value() {
...
/// for "no stats persisted"); call [`Self::finish`] to assemble a [`StructArray`] with
/// the canonical [`STATS_COLUMNS_DTYPE`] schema.
pub struct StatsColumnsBuilder {
    n: usize,
    min: VarBinViewBuilder,
    min_exact: BoolBuilder,
    max: VarBinViewBuilder,
    max_exact: BoolBuilder,
    sum: VarBinViewBuilder,
    null_count: PrimitiveBuilder<u64>,
    nan_count: PrimitiveBuilder<u64>,
    uncompressed_size_in_bytes: PrimitiveBuilder<u64>,
    is_constant: BoolBuilder,
    is_sorted: BoolBuilder,
    is_strict_sorted: BoolBuilder,
}

impl StatsColumnsBuilder {
    pub fn with_capacity(n: usize) -> Self {
        let nullable = Nullability::Nullable;
        let bin = || VarBinViewBuilder::with_capacity(DType::Binary(nullable), n);
        let bool_ = || BoolBuilder::with_capacity(nullable, n);
        let u64_ = || PrimitiveBuilder::<u64>::with_capacity(nullable, n);
        Self {
            n: 0,
            min: bin(),
            min_exact: bool_(),
            ...
            null_count: u64_(),
            nan_count: u64_(),
            uncompressed_size_in_bytes: u64_(),
            is_constant: bool_(),
            is_sorted: bool_(),
            is_strict_sorted: bool_(),
        }
    }
    ...
    pub fn push(&mut self, stats: Option<&StatsSet>) {
        ...
        let Some(stats) = stats else {
            return self.push_all_nulls();
        };

        let bool_dtype = DType::Bool(Nullability::NonNullable);
        let u64_dtype: DType = PType::U64.into();

        // min/max: serialize value + record exact flag. `Precision::Absent` → write nulls.
        let min = stats.get(Stat::Min);
        if let Some(v) = min.as_ref().into_inner() {
            let bytes = ScalarValue::to_proto_bytes::<Vec<u8>>(Some(v));
            self.min.append_value(&bytes);
            self.min_exact.append_value(min.is_exact());
        } else {
            self.min.append_null();
            self.min_exact.append_null();
        }
        let max = stats.get(Stat::Max);
        if let Some(v) = max.as_ref().into_inner() {
            let bytes = ScalarValue::to_proto_bytes::<Vec<u8>>(Some(v));
            self.max.append_value(&bytes);
            self.max_exact.append_value(max.is_exact());
        } else {
            self.max.append_null();
            self.max_exact.append_null();
        }
        // sum: exact-only; inexact sums are dropped.
        if let Some(v) = stats.get(Stat::Sum).as_exact() {
            let bytes = ScalarValue::to_proto_bytes::<Vec<u8>>(Some(&v));
            self.sum.append_value(&bytes);
        } else {
            self.sum.append_null();
        }
        push_opt_u64(
            &mut self.null_count,
            stats.get_as::<u64>(Stat::NullCount, &u64_dtype).as_exact(),
        );
        push_opt_u64(
            &mut self.nan_count,
            stats.get_as::<u64>(Stat::NaNCount, &u64_dtype).as_exact(),
        );
        push_opt_u64(
            &mut self.uncompressed_size_in_bytes,
            stats
    ...
    fn push_all_nulls(&mut self) {
        self.min.append_null();
        self.min_exact.append_null();
        self.max.append_null();
        self.max_exact.append_null();
        self.sum.append_null();
        self.null_count.append_null();
        self.nan_count.append_null();
        self.uncompressed_size_in_bytes.append_null();
        self.is_constant.append_null();
        self.is_sorted.append_null();
        self.is_strict_sorted.append_null();
    }

    /// Materialize the accumulated rows into a [`StatsColumns`].
    pub fn finish(mut self) -> VortexResult<StatsColumns> {
        let n = self.n;
        let fields: Vec<ArrayRef> = vec![
            self.min.finish_into_varbinview().into_array(),
            self.min_exact.finish_into_bool().into_array(),
            self.max.finish_into_varbinview().into_array(),
            self.max_exact.finish_into_bool().into_array(),
            self.sum.finish_into_varbinview().into_array(),
            self.null_count.finish_into_primitive().into_array(),
            self.nan_count.finish_into_primitive().into_array(),
            self.uncompressed_size_in_bytes
                .finish_into_primitive()
                .into_array(),
            self.is_constant.finish_into_bool().into_array(),
            self.is_sorted.finish_into_bool().into_array(),
            self.is_strict_sorted.finish_into_bool().into_array(),
        ];
        let names = STATS_FIELDS
            .iter()
...
/// Canonical schema of the `nodes` struct that backs a [`ColumnarArrayTree`]
/// (one row per `ArrayNode` in pre-order).
///
...
/// [`STATS_COLUMNS_DTYPE`]).
pub static NODES_COLUMNS_DTYPE: LazyLock<DType> = LazyLock::new(|| {
    ...
    DType::Struct(
        StructFields::new(
            ...
            vec![
                ...
                STATS_COLUMNS_DTYPE.clone(),             // stats
            ],
        ),
        Nullability::NonNullable,
    )
