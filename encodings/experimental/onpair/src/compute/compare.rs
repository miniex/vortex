...
impl CompareKernel for OnPair {
    fn compare(
        ...
        // The only comparison OnPair can push down without decompressing is
        // against the empty string, which is decided by the per-row uncompressed
        // lengths alone. `onpair` exposes no way to re-encode a value against an
        // existing dictionary, so any other constant falls back to canonical.
