...
fn take_indices<T: AsPrimitive<usize>>(
    ...
    let physical_indices = match_each_unsigned_integer_ptype!(ends.ptype(), |I| {
        ...
        if u32::try_from(ends.len()).is_ok() {
            PrimitiveArray::new(
    ...
    usize: AsPrimitive<O>,
{
    ...
    // Sorted indices never reach this point when the condition holds: runs <= array_len <=
    // 8 * count implies the sorted linear scan above was taken.
    ...
    usize: AsPrimitive<O>,
{
    ...
    match validity_mask.bit_buffer() {
        AllOr::All => indices.iter().map(|idx| table[idx.as_()]).collect(),
        AllOr::None => Buffer::zeroed(indices.len()),
        AllOr::Some(validity) => validity
            .iter()
            .zip(indices.iter())
